...
if (isset($_GET["a"])) {
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    if ($_GET["a"] == "lista_cat") {
        ...
        if ($pesquisa != "") {
            $where .= "WHERE cat_desc LIKE '%{$pesquisa}%' ";
        }

        $res = $db->select("SELECT * FROM tb_categorias
                                {$where} ORDER BY cat_desc");

        if (count($res) > 0) {
            echo '<table class="table align-items-center mb-0">';
            echo '  <thead>';
            echo '      <tr>';
            echo '          <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 text-left">Categoria</th>';
            ...
            foreach($res as $r){
                ...
                echo '      <span class="text-secondary text-xs font-weight-bold" style="padding-left:15px;">'.$r["cat_desc"].'</span>';
                echo '  </td>';
                echo '  <td class="align-middle">';
                echo '      <i title="Editar" onclick="get_item(\'' . $r["cat_id"] . '\')" class="fa fa-edit" style="cursor: pointer"></i>';
                echo '  </td>';
                echo '  <td class="align-middle">';
                echo '      <i title="Deletar" onclick="del_item(\'' . $r["cat_id"] . '\')" class="fa fa-trash" style="cursor: pointer"></i>';
                echo '  </td>';
                echo '</tr>';
            }
            echo '  </tbody>';
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    if ($_GET["a"] == "inclui_cat") {

        $desc = $_POST["desc"];
        
        $res = $db->_exec("INSERT INTO tb_categorias (cat_desc) VALUES ('{$desc}')");
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    if ($_GET["a"] == "edita_cat") {
        ...
        $res = $db->_exec("UPDATE tb_categorias SET cat_desc = '{$desc}' WHERE cat_id = $id");
    ...
    * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    if ($_GET["a"] == "del_cat") {
        ...
        $del = $db->_exec("DELETE FROM tb_categorias WHERE cat_id = {$id}");
            ...
            <div class="modal-content">
                <div class="modal-header" style="align-items: center">
                    <div style="display: flex; align-items: center">
                        ...
                        <div>
                            <h5 id="tit_frm_formul" class="modal-title">Incluir Categoria</h5>
                        </div>
                    </div>
                ...
                <div class="modal-body modal-dialog-scrollable">
                    <form id="frm_general" name="frm_general" class="col">
                        <div class="row mb-3">
                            <div class="col-4">
                                <label for="frm_categoria" class="form-label">Categoria:</label>
                                <div class="input-group input-group-outline">
                                    <input type="text" class="form-control" id="frm_categoria" placeholder="Ex: Português">
                                </div>
                            </div>
                        </div>
                    </form>
            ...
            <div class="modal-content">
                <div class="modal-header" style="align-items: center">
                    <div style="display: flex; align-items: center">
                        ...
                        <div>
                            <h5 id="div_edit_title">Editar Categoria</h5>
                        </div>
                    </div>
                ...
                <div class="modal-body modal-dialog-scrollable">
                    <form id="frm_general" name="frm_general" class="col">
                        <div class="row mb-3">
                            <div class="col-4">
                                <label for="frm_categoria_edit" class="form-label">Categoria:</label>
                                <div class="input-group input-group-outline">
                                    <input id="frm_id_edit" hidden>
                                    <input type="text" class="form-control" id="frm_categoria_edit">
                                </div>
                            </div>
                        </div>
                    </form>
...
<script type="text/javascript">
    ...
    const editUsu = (countarray, iditens, idPed) => {
        ...
        if (confirm("Confirma a edição da Categoria?")) {
            if (ajax_div) {
                ajax_div.abort();
            }
            ...
            ajax_div = $.ajax({
                cache: false,
                async: true,
                url: '?uid=<?php echo $_COOKIE['idUsuario']; ?>&a=edita_cat',
                type: 'post',
                data: {
                    desc: $('#frm_categoria_edit').val(),
                    id: $('#frm_id_edit').val(),
                },
                beforeSend: function() {
    ...
    const get_item = (id) => {
        ...
        ajax_div = $.ajax({
            cache: false,
            async: true,
            url: '?uid=<?php echo $_COOKIE['idUsuario']; ?>&a=get_cat',
            type: 'post',
            data: {
                id: id,
            },
            ...
            success: function retorno_ajax(retorno) {
                ...
                $("#frm_categoria_edit").val(obj[0].cat_desc);
            }
        });
    }
    ...
    function del_item(id) {
        if (confirm("Deseja excluir a categoria?")) {
            ...
            ajax_div = $.ajax({
                cache: false,
                async: true,
                url: '?uid=<?php echo $_COOKIE['idUsuario']; ?>&a=del_cat',
                type: 'post',
                data: {
                    id: id,
                },
